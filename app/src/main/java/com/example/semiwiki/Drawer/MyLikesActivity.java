...
public class MyLikesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        header.bindDrawer(binding.drawerLayout, binding.navView, new HeaderView.DrawerActions() {
            @Override public void onClickMyPosts() { startActivity(new Intent(MyLikesActivity.this, MyPostsActivity.class)); }
        ...
        header.setListener(new HeaderView.Listener() {
            ...
            public void onSearchSubmit(String keyword) {
                ...
                Intent intent = new Intent(MyLikesActivity.this, com.example.semiwiki.Board.BoardActivity.class);
        ...
        userService.getUserLikedPosts("Bearer " + token, accountId, 0, 20)
                .enqueue(new Callback<List<BoardListItemDTO>>() {
                    ...
                    public void onResponse(Call<List<BoardListItemDTO>> call, Response<List<BoardListItemDTO>> resp) {
                        ...
                        if (resp.code() == 204) { adapter.submitList(Collections.emptyList()); showEmpty(); return; }
                        if (resp.isSuccessful() && resp.body() != null) {
                            adapter.submitList(BoardMappers.toBoardItems(resp.body()));
                            showList();
                        } else {
                            Log.e(TAG, "liked 목록 실패: " + resp.code());
                            Toast.makeText(MyLikesActivity.this, "목록을 불러오지 못했어요 (" + resp.code() + ")", Toast.LENGTH_SHORT).show();
                            showEmpty();
                        }
                    }
                    @Override
                    public void onFailure(Call<List<BoardListItemDTO>> call, Throwable t) {
                        ...
                        Toast.makeText(MyLikesActivity.this, "네트워크 오류가 발생했어요", Toast.LENGTH_SHORT).show();
                        showEmpty();
                    }
                });
    }
